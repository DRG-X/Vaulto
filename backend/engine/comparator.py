...
async def fetch_pools(amount: Decimal, currency_from: str, currency_to: str) -> QuotePools:
    """
    Fetch and normalize every provider's options. The expensive half.
    ...
    raws: List[RawQuote] = list(await asyncio.gather(*[
        _safe_fetch(p, amount, currency_from, currency_to) for p in ALL_PROVIDERS
    ]))
    ...
    for raw in raws:
        ...
        normalized = [norm.normalize_quote(option, amount, mid_rate) for option in candidates]
    ...
    return QuotePools(pools=pools, errors=errors, mid_rate=mid_rate, mid_source=mid_source)
...
def rank_pools(data: QuotePools, request: CompareRequest) -> CompareResponse:
    ...
    if not quotes and not filtered_out:
        reasons = "; ".join(f"{q.provider}: {q.error}" for q in data.errors) or "unknown"
        raise ValueError(
            f"No provider returned a usable quote for "
            f"{request.currency_from}->{request.currency_to}. {reasons}"
        )
